...
const Collaborations = ({ itemId }) => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchcollabration(sendData));
  }, [searchTrigger, dispatch, selectedStatus, itemId]);
  useEffect(() => {
    if (itemId2) {
      dispatch(fetchcollabrationDetails(itemId2));
    }
  }, [dispatch, itemId2]);

  const handleAddPost = () => {
    localStorage.setItem("itemId", itemId);
    router.push(`/project/add-collaborations`);
  };
    ...
    <div>
      <div className="w-full h-[44px] mt-6  flex justify-between items-center px-2 sm:px-4  ">
        ...
        <div className="flex">
          ...
          <p
            className={`${
              collabrationLoading?.loading && "rotate_01"
      ...
      <DrawerCollaboration
        ...
        itemId2={itemId}
        itemId={itemId2}
        details={collabrationDetailsData}
      />
    </div>
  );
};
